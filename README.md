...
This repo holds the code for automating the process of setting up a device access to the LoRa gateway using the ChirpStack web UI. It is still required to configure the gateway and device profile manually, but device addition and app-key lease will be done automatically, which is especially helpful with 60> devices.
...
Scripts overview. (Scripts don't handle it on their own, it is done through the robot tasks and python scripts, so running the robot command directly has the same effect as running the shell script, which serves only as the simplification of the robot command.)
